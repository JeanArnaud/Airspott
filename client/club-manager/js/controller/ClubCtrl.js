angular.module('com.airspott.club')
    ...
    [
        ...
        function ($rootScope, $scope, $location, Club, Customer, $log, Offer, Upload, Currency, $http, $stateParams, $state, $compile)
        {
            ...
            {
                $scope.data = Club.find({filter : {where : {managerId : Customer.getCurrentId()}}}, function (club)
                {
                    $scope.clubid = club[0].id;
                    $scope.clubdetail = club[0];
                    $scope.loading = false;
                    if(typeof(club[0].step) != 'undefined')
                    {
                        $scope.step = club[0].step;
                    }
                   
                    if(typeof(club[0].coverPhoto) != 'undefined')
                    {
                        $scope.coverPhoto = club[0].coverPhoto;
                    }
                    ...
                    $http.get("/api/Clubs/"+$scope.clubid+"/billing").success(function(bank)
                    {
                        $scope.bill = bank;                        
                    });

                    if(typeof(club[0].activities) != 'undefined' && angular.isArray(club[0].activities))
                    {
                        $scope.cluboffers = club[0].activities;
